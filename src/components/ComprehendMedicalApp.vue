...
export default {
  ...
  methods: {
    ...
    async fetchChatbotMessages() {
      ...
      if (userId) {
        try {
          const response = await axios.get(`http://127.0.0.1:5000/api/get_chatbot_messages/${this.userID}`);
    ...
    async submitFormWithPage(page) {
      ...
      try {
        ...
        await axios.post('http://127.0.0.1:5000/api/add_chatbot_message', userMessage);
        this.messages.push({
          message_type: 'user',
          content: this.description,
        ...
        `;
          ...
          await axios.post('http://127.0.0.1:5000/api/add_chatbot_message', botMessage);
          this.messages.push({
            message_type: 'bot',
            content: 'Hospital charges have been fetched successfully. Please see below for details.',
            timestamp: new Date().toLocaleTimeString(),
